...
async function main(args) {
  ...
  const checkpoint = join(out, "calibration-v3.partial.jsonl");
...
async function ensureTelemetryProbe({ state, hints, bundle, env, timeoutMs, out }) {
  const path = join(out, "telemetry-probe-v3.json");
  if (existsSync(path)) {
...
async function buildBundle(out) {
  ...
  const file = join(out, "kernel-v1-candidate-v3.mjs");
...
function writeCalibrationSummary(checkpoint, state, hints, out) {
  ...
  writeJson(join(out, "calibration-v3.summary.json"), {
