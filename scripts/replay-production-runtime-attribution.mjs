...
const VALIDATOR_ONLY_CROSS_COMMIT_PATHS=new Set(["scripts/validate-production-runtime-full.mjs","scripts/validate-production-runtime-v1-review.mjs","scripts/prepare-production-runtime-v1-selection.mjs","scripts/replay-production-runtime-attribution.mjs","package.json",".github/workflows/optimizer-saas-hardening.yml","research/optimizer/RUNTIME_ATTRIBUTION_MILESTONE_2026-09-24.md"]);
function assertRuntimeCompatible(sourceGit,replayGit){if(!sourceGit||!replayGit||sourceGit===replayGit)return;const diff=gitCommand(["diff","--name-only",sourceGit+".."+replayGit]);const paths=diff.split(/\r?\n/).filter(Boolean);if(!paths.length||paths.every((path)=>VALIDATOR_ONLY_CROSS_COMMIT_PATHS.has(path)))return;throw new Error("El replay cambiaria el runtime respecto del holdout "+sourceGit+". Archivos distintos: "+paths.join(", "));}
