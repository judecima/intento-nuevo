...
#[serde(rename_all = "camelCase")]
struct DualSelectionOutput {
    greedy_best: Option<PackOutput>,
    beam_candidates: Vec<PackOutput>,
}

#[napi(js_name = "packBoardLegacyDualSelection")]
pub fn pack_board_legacy_dual_selection(
    pieces_json: String,
    requests_json: String,
    tolerance: f64,
    beam_width: u32,
    ...
    let requests: Vec<PackBatchRequest> = serde_json::from_str(&requests_json)
        .map_err(|e| Error::new(Status::InvalidArg, format!("invalid batch requests JSON: {e}")))?;
    if requests.is_empty() {
        return serde_json::to_string(&DualSelectionOutput {
            greedy_best: None,
    ...
    // Exact greedy selector contract.
    let mut best_close: Option<usize> = None;
    for (index, output) in outputs.iter().enumerate() {
        if output.colocadas.len() != inputs.len() { continue; }
        match best_close {
    ...
    let greedy_index = if let Some(index) = best_close {
        Some(index)
    } else {
        let max_area = outputs.iter().fold(0.0_f64, |acc, output| acc.max(output.area));
        let threshold = max_area * (1.0 - tolerance);
    ...
    // Exact beam selector contract, from the same already-computed outputs.
    ...
    let board_area = quality_opts.ancho_util * quality_opts.alto_util;
    let mut beam_indices: Vec<usize> = dedup.into_values().collect();
    beam_indices.sort_by(|a_index, b_index| {
    ...
    let limit = usize::max((beam_width as usize).saturating_mul(3), beam_width as usize);
    beam_indices.truncate(limit);

    let greedy_best = greedy_index.map(|index| outputs[index].clone());
    let beam_candidates = beam_indices.into_iter().map(|index| outputs[index].clone()).collect();

    serde_json::to_string(&DualSelectionOutput {
        greedy_best,
        beam_candidates,
    })
    .map_err(|e| Error::new(Status::GenericFailure, format!("serialize dual selection: {e}")))
