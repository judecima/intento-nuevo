...
const VARIANTS = {
  control: { usarMaster: true, usarMultiSlice: true },
  no_multislice: { usarMaster: true, usarMultiSlice: false },
  no_master: { usarMaster: false, usarMultiSlice: true },
  no_master_no_multislice: { usarMaster: false, usarMultiSlice: false },
};
...
function configFor(problem, variant) {
  ...
  return {
    ...
    restoMax: 400,
    usarOneBoard: true,
    usarMaster: variant.usarMaster,
    usarMultiSlice: variant.usarMultiSlice,
...
function runVariant(name) {
  ...
  for (let i = 0; i < TARGETS.length; i++) {
    const [file, expectedPiecesHistorical] = TARGETS[i];
    ...
    const config = configFor(problem, variant);
  ...
  const payload = {
    schema: "optimizer-p99-tail-ablation-v1",
    variant: name,
  ...
  fs.writeFileSync(path.join(OUT, "p99-tail-" + name + ".json"), JSON.stringify(payload, null, 2) + "\n");
}
...
function report() {
  ...
  const runs = Object.fromEntries(Object.keys(VARIANTS).map((name) => [
    name,
    loadJson(path.join(OUT, "p99-tail-" + name + ".json")),
  ]));
  ...
  const result = {
    schema: "optimizer-p99-tail-ablation-report-v1",
    generatedAt: new Date().toISOString(),
    sourceSha: process.env.GITHUB_SHA || null,
    scope: {
      cases: TARGETS.length,
      matching: "order/file + exact historical piece count; ambiguity fails closed",
      note: "Targeted diagnostic only. It does not authorize a global stage disable.",
    },
    summary,
    perCase,
  };

  fs.writeFileSync(path.join(OUT, "p99-tail-ablation-results.json"), JSON.stringify(result, null, 2) + "\n");
  console.log("P99_TAIL_ABLATION " + JSON.stringify(result));
}
