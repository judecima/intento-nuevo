...
const OUT_PATH = path.join(OUT_DIR, "MASTER_P9_GAP1_2026-09-21.json");
...
// Learned from full40 provenance of the three historical gap=1 Master winners.
const P9 = Object.freeze([0,1,2,5,6,17,20,22,36]);
const THRESHOLD = Number(process.env.MASTER_GATE_MULT || 4.75);
const MAX_CASES = Number(process.env.P9_MAX_CASES || 24);
const MAX_PIECES = Number(process.env.P9_MAX_PIECES || 180);
const FULL_MS = Number(process.env.P9_FULL_MS || 8000);
const EARLY_NODES = Number(process.env.P9_EARLY_NODES || 200000);
const EARLY_WATCHDOG_MS = Number(process.env.P9_EARLY_WATCHDOG_MS || 1500);
const FULL_NODES = Number(process.env.P9_FULL_NODES || 1600000);
const FULL_WATCHDOG_MS = Number(process.env.P9_FULL_WATCHDOG_MS || 12000);
...
function runStaged(row) {
  ...
  gen.execute(P9);
  const p9Patterns=gen.patterns(P9);
  const p9GenerationWallMs=performance.now()-p9Start;
  const early=solve(p9Patterns.concat(mono),lines,area,prePlan.resumen.placas,EARLY_NODES,EARLY_WATCHDOG_MS);
  ...
  if (certifiedEarly) {
    return {
      preMs, preBoards:prePlan.resumen.placas, cota:pre.cota,
      stoppedEarly:true, executedRounds:P9.length,
      p9Patterns:p9Patterns.length, monotypes:mono.length, monoMs,
      p9GenerationWallMs, generationCpuMs:gen.generationCpuMs,
  ...
  return {
    ...
    p9Patterns:p9Patterns.length, allPatterns:allPatterns.length, monotypes:mono.length, monoMs,
    p9GenerationWallMs, fallbackGenerationWallMs, generationCpuMs:gen.generationCpuMs,
    earlySolveMs:early.solveMs, earlyNodes:early.sol?.nodos ?? null, earlyExhausted:early.sol?.agotado ?? null,
...
function runPair(row, historical, index) {
  ...
  const candidateMasterWorkMs=
    num(candidate.monoMs)+num(candidate.p9GenerationWallMs)+num(candidate.earlySolveMs)+
...
function main(){
  ...
  for(let i=0;i<chosen.length;i++){
    ...
    records.push(rec);
    console.log("P9_PAIR",JSON.stringify({
      ...
      p9Patterns:rec.candidate.p9Patterns,allPatterns:rec.candidate.allPatterns ?? null,
    }));
  }
  ...
  const summary={
    schema:"master-p9-gap1-provenance-v1",
  ...
  console.log("P9_SUMMARY",JSON.stringify({pass:summary.pass,counts:summary.counts,timing:summary.timing}));
