...
function assertHistoricalWins(){
 ...
 const lost=wins.filter(r=>Number(r.pieces)<MIN_PIECES || Number(r.pieces)>MAX_PIECES);
 if(lost.length) throw new Error("envelope would suppress known wins: "+lost.map(r=>r.file).join(","));
...
function runShard(){
 ...
 fs.writeFileSync(path.join(OUT,"envelope200500-shard-"+SHARD_INDEX+".json"),JSON.stringify({shard:SHARD_INDEX,total:SHARD_TOTAL,historicalWins,rows},null,2)+"\n");
}
...
function report(){
 ...
 const files=fs.readdirSync(OUT).filter(x=>/^envelope200500-shard-\d+\.json$/.test(x));
 ...
 fs.writeFileSync(path.join(OUT,"envelope200500-results.json"),JSON.stringify(result,null,2)+"\n");
 console.log("MULTISLICE_ENVELOPE "+JSON.stringify(result));
 if(invalid.length||digestDiff.length||boardReg.length||remnantReg.length) throw new Error("quality gate failed");
 if(!result.historicalWins.allPreservedByEnvelope) throw new Error("historical win preservation failed");
}
