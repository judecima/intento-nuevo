...
const OUT=__dirname;
const V10=path.join(REPO,"experiencia/v5/baseline-v10-5000-2000-r1.json");
const CANONICAL=path.join(REPO,"experiencia/canonical_cases.json");
const TEST_START=1600,TEST_END=2000;
const MIN_PIECES=200,MAX_PIECES=500;
...
function resolveExact(idx,h){
  const cs=idx.get(orderNumber(h.file))||[];
  const ex=cs.filter(e=>countPieces(e)===Number(h.pieces));
...
function problem(e,h){
  ...
  if(!(width>0)||!(height>0)||pieces.some(p=>!(p.base>0)||!(p.altura>0)||!(p.cant>0)))throw new Error(h.file+": invalid geometry");
  return {file:h.file,order:h.order,width,height,saw,trimX,trimY,directional,pieces,historicalBoards:h.boards,historicalMs:h.totalMs};
}
function prepare(){
  const raw=loadJson(V10);
  const rows=(raw.records||raw).filter(r=>r.ok).sort((a,b)=>(a.order??0)-(b.order??0));
  if(rows.length<TEST_END)throw new Error("need >=2000 v10 rows, got "+rows.length);
  const test=rows.slice(TEST_START,TEST_END);
  const idx=indexCorpus(unwrap(loadJson(CANONICAL)));
  const matched=[],unmatched=[];
  for(const h of test){
    const r=resolveExact(idx,h);
    if(!r.entry){
      unmatched.push({file:h.file,order:h.order,pieces:h.pieces,candidates:r.candidates,exactCount:r.exactCount,counts:r.counts});
      continue;
    }
    matched.push(problem(r.entry,h));
  }
  return {test,matched,unmatched};
}
...
function configFor(p,candidate){
...
function runOne(p,candidate){
  ...
  const cfg=configFor(p,candidate);
  const expected=p.pieces.reduce((s,x)=>s+x.cant,0);
  const t0=process.hrtime.bigint();let result=null,error=null;
  try{result=optimizarV10(lines,cfg,nuevasMetricas());}catch(e){error=String(e?.stack||e?.message||e).slice(0,1200);}
  ...
  return {
    ...
    multiMs:Number(result?.metricas?.multislice?.ms||0),multiActivations:Number(result?.metricas?.multislice?.activaciones||0),multiWins:Number(result?.metricas?.multislice?.ganancias||0),
    masterMs:Number(result?.metricas?.master?.ms||0),compactMs:Number(result?.metricas?.compactacion?.ms||0),
  };
}
function runShard(){
  process.env.OPTIMIZER_RUST_MASTER_ROUND_REUSE_EXPERIMENTAL="0";
  process.env.OPTIMIZER_PACKING_CACHE_KEY_V2_EXPERIMENTAL="0";
  const prep=prepare(),rows=[];
  for(let i=0;i<prep.matched.length;i++){
    if(i%SHARD_TOTAL!==SHARD_INDEX)continue;
    const p=prep.matched[i],order=(i%2===0)?[false,true]:[true,false];
    const pair={file:p.file,orderId:p.order,pieces:p.pieces.reduce((s,x)=>s+x.cant,0),globalIndex:i,runOrder:order.map(x=>x?"candidate":"control")};
    for(const candidate of order){
      const r=runOne(p,candidate);
      pair[candidate?"candidate":"control"]=r;
      console.log("shard="+SHARD_INDEX+" "+p.file+" "+r.mode+" pieces="+pair.pieces+" boards="+r.boards+" ms="+r.elapsedMs.toFixed(1)+" multi="+r.multiMs.toFixed(1));
    }
    rows.push(pair);
  }
  fs.writeFileSync(path.join(OUT,"final-shard-"+SHARD_INDEX+".json"),JSON.stringify({
    shard:SHARD_INDEX,total:SHARD_TOTAL,testCases:prep.test.length,matched:prep.matched.length,unmatched:prep.unmatched.length,unmatchedCases:prep.unmatched,rows
  },null,2)+"\n");
}
...
function report(){
  ...
  const files=fs.readdirSync(OUT).filter(x=>/^final-shard-\d+\.json$/.test(x));
  const payloads=files.map(f=>loadJson(path.join(OUT,f)));
  if(!payloads.length)throw new Error("no shard payloads");
  const expectedMatched=payloads[0].matched,expectedTest=payloads[0].testCases,expectedUnmatched=payloads[0].unmatched;
  ...
  const result={
    schema:"optimizer-multislice-envelope-final-holdout-v1",
    minPieces:MIN_PIECES,maxPieces:MAX_PIECES,
    cohort:{historicalHoldout:expectedTest,exactMatched:expectedMatched,unmatched:expectedUnmatched,coveragePct:+(100*expectedMatched/Math.max(1,expectedTest)).toFixed(2),
    ...
    topControlTail:rows.slice().sort((x,y)=>y.control.elapsedMs-x.control.elapsedMs).slice(0,20).map(r=>({file:r.file,pieces:r.pieces,controlMs:+r.control.elapsedMs.toFixed(2),candidateMs:+r.candidate.elapsedMs.toFixed(2),speedupPct:red(r.control.elapsedMs,r.candidate.elapsedMs),boards:r.control.boards,controlMultiMs:+r.control.multiMs.toFixed(2),candidateMultiMs:+r.candidate.multiMs.toFixed(2)}))
  };
  fs.writeFileSync(path.join(OUT,"final-results.json"),JSON.stringify(result,null,2)+"\n");
  console.log("MULTISLICE_ENVELOPE_FINAL "+JSON.stringify(result));
