...
function shard(){
 ...
 for(let i=SHARD_INDEX;i<corpus.length;i+=SHARD_TOTAL){
  ...
  if(!isAffected(p)){counts.single++;continue;}
  counts.paired++;
  ...
  console.log("MID_REPEAT_AFFECTED "+JSON.stringify({order:id.order,types:p.types,pieces:p.pieces,base:base.boards,cand:cand.boards,delta:row.boardDelta,samePlan:row.samePlan,sameQuality:row.sameQuality,baseMasterActive:base.masterActivations,candMasterActive:cand.masterActivations,baseCpu:base.cpuMs,candCpu:cand.cpuMs}));
 }
 fs.writeFileSync(path.join(__dirname,"master-mid-repeat-p3-affected-e2e-shard-"+SHARD_INDEX+".json"),JSON.stringify({schema:"master-mid-repeat-p3-affected-e2e-shard-v1",shard:SHARD_INDEX,shardTotal:SHARD_TOTAL,counts,rows},null,2)+"\n");
}
...
function report(){
 const expected=Number(process.env.SHARD_TOTAL||32),files=fs.readdirSync(__dirname).filter(x=>/^master-mid-repeat-p3-affected-e2e-shard-\d+\.json$/.test(x));
 if(files.length!==expected)throw new Error("expected "+expected+" shards got "+files.length);
 const shards=files.map(f=>JSON.parse(fs.readFileSync(path.join(__dirname,f),"utf8"))),rows=shards.flatMap(s=>s.rows||[]),good=rows.filter(r=>r.base?.ok&&r.cand?.ok),paired=good;
 ...
 const out={schema:"master-mid-repeat-p3-affected-e2e-results-v1",cases:good.length,pairedCases:paired.length,pairedMasterActive:paired.filter(r=>(r.base?.masterActivations||0)>0).length,invalid:rows.length-good.length,plan:{equal:paired.filter(r=>r.samePlan).length,different:paired.filter(r=>!r.samePlan).map(r=>r.order)},boards:{losses:losses.map(r=>({order:r.order,base:r.base.boards,cand:r.cand.boards})),gains:gains.map(r=>({order:r.order,base:r.base.boards,cand:r.cand.boards})),same:same.length},quality:{equalBoardQualityDiffs:qualityDiff.map(r=>r.order)},cpu:{baseTotalMs:baseCpu,candidateTotalMs:candCpu,savingPct:baseCpu>0?100*(baseCpu-candCpu)/baseCpu:null,baseP50:quant(good.map(r=>r.base.cpuMs),.5),candP50:quant(good.map(r=>r.cand.cpuMs),.5),baseP95:quant(good.map(r=>r.base.cpuMs),.95),candP95:quant(good.map(r=>r.cand.cpuMs),.95),baseP99:quant(good.map(r=>r.base.cpuMs),.99),candP99:quant(good.map(r=>r.cand.cpuMs),.99)},wall:{baseTotalMs:baseWall,candidateTotalMs:candWall,savingPct:baseWall>0?100*(baseWall-candWall)/baseWall:null},master:{baseMs:sum(good.map(r=>r.base.masterMs)),candMs:sum(good.map(r=>r.cand.masterMs))},rows};
 fs.writeFileSync(path.join(__dirname,"master-mid-repeat-p3-affected-e2e-results.json"),JSON.stringify(out,null,2)+"\n");
 console.log("MID_REPEAT_AFFECTED_SUMMARY "+JSON.stringify({cases:out.cases,pairedCases:out.pairedCases,pairedMasterActive:out.pairedMasterActive,invalid:out.invalid,plan:out.plan,boards:out.boards,quality:out.quality,cpu:out.cpu,wall:out.wall,master:out.master}));
