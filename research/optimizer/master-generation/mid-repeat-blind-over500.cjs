...
function blind(){
 const corpus=loadCorpus(),rows=[],stats={assigned:0,over500:0,over500RuleB:0,active:0,errors:0};
 for(let i=SHARD_INDEX;i<corpus.length;i+=SHARD_TOTAL){
  ...
  if(!(p.types>=20&&p.types<=40&&p.pieces/p.types>=4))continue;stats.over500RuleB++;
  ...
  try{
   ...
   console.log("MID_REPEAT_BLIND_OVER500 "+JSON.stringify({order:id.order,pieces:p.pieces,types:p.types,pre:sc.boards,lb:sc.cota,p3:p3.boards,full:full.boards,loss:p3.boards>full.boards,gain:p3.boards<full.boards}));
  }catch(error){stats.errors++;rows.push({...id,canonicalIndex:i,error:String(error?.stack||error)});}
 }
 fs.writeFileSync(path.join(__dirname,"mid-repeat-blind-over500-shard-"+SHARD_INDEX+".json"),JSON.stringify({schema:"mid-repeat-blind-over500-shard-v1",shard:SHARD_INDEX,shardTotal:SHARD_TOTAL,stats,rows},null,2)+"\n");
}
...
function report(){
 const expected=Number(process.env.SHARD_TOTAL||8),files=fs.readdirSync(__dirname).filter(x=>/^mid-repeat-blind-over500-shard-\d+\.json$/.test(x));
 ...
 const out={schema:"mid-repeat-blind-over500-results-v1",stats:{over500:sum(shards.map(s=>s.stats.over500)),over500RuleB:sum(shards.map(s=>s.stats.over500RuleB)),active:sum(shards.map(s=>s.stats.active)),errors:sum(shards.map(s=>s.stats.errors))},cases:good.length,losses:good.filter(r=>r.loss).map(r=>r.order),gains:good.filter(r=>r.gain).map(r=>r.order),parity:good.filter(r=>r.parity).length,p3CpuMs:sum(good.map(r=>r.p3.totalCpuMs)),fullCpuMs:sum(good.map(r=>r.full.totalCpuMs)),rows};
 out.savingPct=out.fullCpuMs>0?100*(out.fullCpuMs-out.p3CpuMs)/out.fullCpuMs:null;
 fs.writeFileSync(path.join(__dirname,"mid-repeat-blind-over500-results.json"),JSON.stringify(out,null,2)+"\n");
 console.log("MID_REPEAT_BLIND_OVER500_SUMMARY "+JSON.stringify({stats:out.stats,cases:out.cases,losses:out.losses,gains:out.gains,parity:out.parity,savingPct:out.savingPct}));
