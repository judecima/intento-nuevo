...
function run(p,mode){
 const cfg=config(p,mode),cpu0=process.cpuUsage(),t0=process.hrtime.bigint();
 try{
...
function category(p,base){
 if((base?.masterActivations||0)<=0)return "no-master";
 if(p.types>40)return "A-high-types";
 if(p.types>=20&&p.types<=40&&p.pieces/p.types>=4)return "B-mid-repeat";
 return "C-full40-incremental";
}
function qualityCmp(a,b){return JSON.stringify(a)===JSON.stringify(b);}
function shard(){
 const corpus=loadCorpus(),rows=[],counts={assigned:0,valid:0,invalid:0,masterCompared:0,copiedNoMaster:0};
 for(let i=SHARD_INDEX;i<corpus.length;i+=SHARD_TOTAL){
  ...
  const base=run(p,"v2");
  let cand;
  if(base.ok&&(base.masterActivations||0)===0){cand={...base};counts.copiedNoMaster++;}
  else{
   counts.masterCompared++;
   cand=run(p,"v3");
  }
  const cat=category(p,base);
  const row={...id,canonicalIndex:i,pieces:p.pieces,typeCount:p.types,piecesPerType:p.types?p.pieces/p.types:null,category:cat,base,cand,boardDelta:(cand.boards??Infinity)-(base.boards??Infinity),sameBoards:cand.boards===base.boards,samePlan:cand.planDigest===base.planDigest,sameQuality:qualityCmp(cand.quality,base.quality),cpuSavingMs:base.cpuMs-cand.cpuMs,wallSavingMs:base.wallMs-cand.wallMs};
  rows.push(row);if(base.ok&&cand.ok)counts.valid++;else counts.invalid++;
  if(cat!=="no-master")console.log("INDUSTRIAL_V3 "+JSON.stringify({order:id.order,category:cat,types:p.types,pieces:p.pieces,base:base.boards,cand:cand.boards,delta:row.boardDelta,sameQuality:row.sameQuality,baseCpu:base.cpuMs,candCpu:cand.cpuMs}));
 }
 fs.writeFileSync(path.join(__dirname,"master-industrial-v3-e2e-shard-"+SHARD_INDEX+".json"),JSON.stringify({schema:"master-industrial-v3-e2e-shard-v1",shard:SHARD_INDEX,shardTotal:SHARD_TOTAL,counts,rows},null,2)+"\n");
}
...
function summarize(rows){
 const good=rows.filter(r=>r.base?.ok&&r.cand?.ok),sum=xs=>xs.reduce((s,x)=>s+(Number.isFinite(x)?x:0),0);
 ...
 const losses=good.filter(r=>r.boardDelta>0),gains=good.filter(r=>r.boardDelta<0),same=good.filter(r=>r.boardDelta===0),qd=good.filter(r=>r.boardDelta===0&&!r.sameQuality);
 return {cases:good.length,invalid:rows.length-good.length,boards:{losses:losses.map(r=>({order:r.order,base:r.base.boards,cand:r.cand.boards})),gains:gains.map(r=>({order:r.order,base:r.base.boards,cand:r.cand.boards})),same:same.length},quality:{equalBoardQualityDiffs:qd.map(r=>r.order)},cpu:{baseTotalMs:baseCpu,candidateTotalMs:candCpu,savingPct:baseCpu?100*(baseCpu-candCpu)/baseCpu:null,baseP50:quant(good.map(r=>r.base.cpuMs),.5),candP50:quant(good.map(r=>r.cand.cpuMs),.5),baseP95:quant(good.map(r=>r.base.cpuMs),.95),candP95:quant(good.map(r=>r.cand.cpuMs),.95),baseP99:quant(good.map(r=>r.base.cpuMs),.99),candP99:quant(good.map(r=>r.cand.cpuMs),.99)},wall:{baseTotalMs:baseWall,candidateTotalMs:candWall,savingPct:baseWall?100*(baseWall-candWall)/baseWall:null},master:{baseMs:sum(good.map(r=>r.base.masterMs)),candMs:sum(good.map(r=>r.cand.masterMs))}};
}
function report(){
 const expected=Number(process.env.SHARD_TOTAL||32),files=fs.readdirSync(__dirname).filter(x=>/^master-industrial-v3-e2e-shard-\d+\.json$/.test(x));
 ...
 const out={schema:"master-industrial-v3-e2e-results-v1",global:summarize(rows),categories:{}};
 for(const cat of ["A-high-types","B-mid-repeat","C-full40-incremental","no-master"])out.categories[cat]=summarize(rows.filter(r=>r.category===cat));
 out.rows=rows;
 fs.writeFileSync(path.join(__dirname,"master-industrial-v3-e2e-results.json"),JSON.stringify(out,null,2)+"\n");
 console.log("INDUSTRIAL_V3_SUMMARY "+JSON.stringify({global:out.global,categories:out.categories}));
 const bad=[...out.global.boards.losses,...out.global.quality.equalBoardQualityDiffs];
