...
function selectTargets(){
 ...
 for(const id of ["4050594","4059776"]){const x=matched.find(t=>t.file.includes(id));if(x&&!selected.some(t=>t.file===x.file))selected.push(x);}
...
function child(file,pieces,candidate){
 ...
 if(candidate)process.env.OPTIMIZER_RUST_LEAN_BEAM_EXPERIMENTAL="1";else delete process.env.OPTIMIZER_RUST_LEAN_BEAM_EXPERIMENTAL;
...
function shard(){
 ...
 fs.writeFileSync(path.join(OUT,"lean-beam-e2e-shard-"+SHARD_INDEX+".json"),JSON.stringify({shard:SHARD_INDEX,total:SHARD_TOTAL,meta:{selected:meta.selected.length,matchedCount:meta.matchedCount,masterCount:meta.masterCount,unmatchedCount:meta.unmatchedCount},rows},null,2)+"\n");
}
...
function report(){
 ...
 const files=fs.readdirSync(OUT).filter(x=>/^lean-beam-e2e-shard-\d+\.json$/.test(x)),payloads=files.map(f=>JSON.parse(fs.readFileSync(path.join(OUT,f),"utf8"))),rows=payloads.flatMap(x=>x.rows).sort((a,b)=>a.index-b.index);
 ...
 const result={schema:"master-40-lean-beam-e2e-v1",rounds:40,cohort:payloads[0].meta,
 ...
 fs.writeFileSync(path.join(OUT,"lean-beam-e2e-results.json"),JSON.stringify(result,null,2)+"\n");console.log("MASTER40_LEAN_BEAM_E2E "+JSON.stringify(result));
