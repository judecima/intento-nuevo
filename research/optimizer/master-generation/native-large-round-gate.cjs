...
function parent(){
 ...
 const result={schema:"master-40-round-native-large-round-v1",rounds:40,rows,summary:{controlMs:+control.toFixed(2),candidateMs:+candidate.toFixed(2),speedupPct:pct(control,candidate),poolParity:rows.every(r=>r.poolParity)}};
 fs.mkdirSync(OUT,{recursive:true});fs.writeFileSync(path.join(OUT,"native-large-round-results.json"),JSON.stringify(result,null,2)+"\n");
 console.log("MASTER40_NATIVE_LARGE_ROUND "+JSON.stringify(result));
