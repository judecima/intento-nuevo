...
function shard(){
 ...
 for(let i=SHARD_INDEX;i<wins.length;i+=SHARD_TOTAL){
   ...
   console.log("INCREMENTAL_CURVE_CASE "+JSON.stringify({order:w.order,parity,normal:{b:normal.earliestBoardMatch,p:normal.earliestPhysicalMatch,cpu:normal.cpuToBoardMatch},guided:{b:guided.earliestBoardMatch,p:guided.earliestPhysicalMatch,cpu:guided.cpuToBoardMatch}}));
 }
 fs.writeFileSync(path.join(__dirname,"master40-incremental-curve-shard-"+SHARD_INDEX+".json"),JSON.stringify({schema:"master40-incremental-curve-shard-v1",shard:SHARD_INDEX,shardTotal:SHARD_TOTAL,rows},null,2)+"\n");
}
...
function report(){
 const expected=Number(process.env.SHARD_TOTAL||8),files=fs.readdirSync(__dirname).filter(x=>/^master40-incremental-curve-shard-\d+\.json$/.test(x));
 ...
 const result={schema:"master40-incremental-curve-benchmark-v1",cases:rows.length,parityCases:good.length,parityFailures:bad.map(r=>r.order),
 ...
 fs.writeFileSync(path.join(__dirname,"master40-incremental-curve-results.json"),JSON.stringify(result,null,2)+"\n");
 console.log("MASTER40_INCREMENTAL_CURVE "+JSON.stringify({cases:result.cases,parityFailures:result.parityFailures,boardCurve:result.boardCurve,physicalCurve:result.physicalCurve,cpu:result.cpuToBoardMatch,scheduling:result.scheduling}));
