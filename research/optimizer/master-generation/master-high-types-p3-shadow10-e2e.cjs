...
function run(p,shadowRate){
 const cfg=config(p,shadowRate),cpu0=process.cpuUsage(),t0=process.hrtime.bigint();
 try{
...
function shard(){
 ...
 for(let i=SHARD_INDEX;i<corpus.length;i+=SHARD_TOTAL){
  ...
  if(p.types>40){
   counts.paired++;
   const firstShadow=(i%2===1),a=firstShadow?run(p,.10):run(p,0),b=firstShadow?run(p,0):run(p,.10),base=firstShadow?b:a,cand=firstShadow?a:b;
   ...
   console.log("SHADOW10_E2E "+JSON.stringify({order:id.order,types:p.types,base:base.boards,cand:cand.boards,delta:row.boardDelta,sameQuality:row.sameQuality,eligible:cand.shadow?.eligible||0,sampled:cand.shadow?.sampled||0,full40Better:cand.shadow?.full40Better||0,primaryBetter:cand.shadow?.primaryBetter||0,baseCpu:base.cpuMs,candCpu:cand.cpuMs}));
  }else{
   counts.single++;const base=run(p,0),cand={...base};rows.push({...id,canonicalIndex:i,pieces:p.pieces,typeCount:p.types,base,cand,boardDelta:0,sameBoards:true,samePlan:true,sameQuality:true,cpuSavingMs:0,wallSavingMs:0});if(base.ok)counts.valid++;else counts.invalid++;
  }
 }
 fs.writeFileSync(path.join(__dirname,"master-high-types-p3-shadow10-e2e-shard-"+SHARD_INDEX+".json"),JSON.stringify({schema:"master-high-types-p3-shadow10-e2e-shard-v1",shard:SHARD_INDEX,shardTotal:SHARD_TOTAL,counts,rows},null,2)+"\n");
}
...
function report(){
 const expected=Number(process.env.SHARD_TOTAL||32),files=fs.readdirSync(__dirname).filter(x=>/^master-high-types-p3-shadow10-e2e-shard-\d+\.json$/.test(x));
 ...
 const shadowEligible=sum(paired.map(r=>r.cand.shadow?.eligible||0)),shadowSampled=sum(paired.map(r=>r.cand.shadow?.sampled||0)),shadowFull40Better=sum(paired.map(r=>r.cand.shadow?.full40Better||0)),shadowPrimaryBetter=sum(paired.map(r=>r.cand.shadow?.primaryBetter||0)),shadowEqual=sum(paired.map(r=>r.cand.shadow?.equalBoards||0)),shadowMs=sum(paired.map(r=>r.cand.shadow?.ms||0));
 const out={schema:"master-high-types-p3-shadow10-e2e-results-v1",cases:good.length,pairedCases:paired.length,invalid:rows.length-good.length,boards:{losses:losses.map(r=>({order:r.order,base:r.base.boards,cand:r.cand.boards})),gains:gains.map(r=>({order:r.order,base:r.base.boards,cand:r.cand.boards})),same:same.length},quality:{equalBoardQualityDiffs:qualityDiff.map(r=>r.order)},shadow:{eligible:shadowEligible,sampled:shadowSampled,samplePct:shadowEligible?100*shadowSampled/shadowEligible:null,full40Better:shadowFull40Better,primaryBetter:shadowPrimaryBetter,equal:shadowEqual,ms:shadowMs},cpu:{baseTotalMs:baseCpu,candidateTotalMs:candCpu,overheadPct:baseCpu>0?100*(candCpu-baseCpu)/baseCpu:null,baseP50:quant(good.map(r=>r.base.cpuMs),.5),candP50:quant(good.map(r=>r.cand.cpuMs),.5),baseP95:quant(good.map(r=>r.base.cpuMs),.95),candP95:quant(good.map(r=>r.cand.cpuMs),.95),baseP99:quant(good.map(r=>r.base.cpuMs),.99),candP99:quant(good.map(r=>r.cand.cpuMs),.99)},wall:{baseTotalMs:baseWall,candidateTotalMs:candWall,overheadPct:baseWall>0?100*(candWall-baseWall)/baseWall:null},master:{baseMs:sum(good.map(r=>r.base.masterMs)),candMs:sum(good.map(r=>r.cand.masterMs))},rows};
 fs.writeFileSync(path.join(__dirname,"master-high-types-p3-shadow10-e2e-results.json"),JSON.stringify(out,null,2)+"\n");
 console.log("SHADOW10_E2E_SUMMARY "+JSON.stringify({cases:out.cases,pairedCases:out.pairedCases,invalid:out.invalid,boards:out.boards,quality:out.quality,shadow:out.shadow,cpu:out.cpu,wall:out.wall,master:out.master}));
