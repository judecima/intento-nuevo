...
function child(file,candidate){
 ...
 const row={file,pieces:target[1],mode:candidate?"candidate":"control",ms,poolSize:pool.length,digest:patternDigest(pool),rust:cfg._rustPatternGeneratorUsed||null,fallback:Boolean(cfg._rustPatternGeneratorFallback),roundPolicy:cfg._patternMaskPolicy||null,masterContext:cfg._rustMasterContext||null};
...
function parent(){
 ...
 const result={schema:"master-40-round-master-large-context-v1",rounds:40,rows,summary:{controlMs:+control.toFixed(2),candidateMs:+candidate.toFixed(2),speedupPct:pct(control,candidate),poolParity:rows.every(r=>r.poolParity)}};
 fs.mkdirSync(OUT,{recursive:true});fs.writeFileSync(path.join(OUT,"master-large-context-results.json"),JSON.stringify(result,null,2)+"\n");
 console.log("MASTER40_RUST_LARGE_CONTEXT "+JSON.stringify(result));
