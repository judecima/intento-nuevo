...
function child(file,candidate){
 ...
 if(candidate) process.env.OPTIMIZER_RUST_ORDERED_REPS_EXPERIMENTAL="1";
 else delete process.env.OPTIMIZER_RUST_ORDERED_REPS_EXPERIMENTAL;
...
function parent(){
 ...
 const result={schema:"master-40-round-ordered-reps-v1",rounds:40,rows,summary:{controlMs:+control.toFixed(2),candidateMs:+candidate.toFixed(2),speedupPct:pct(control,candidate),poolParity:rows.every(r=>r.poolParity)}};
 fs.mkdirSync(OUT,{recursive:true});fs.writeFileSync(path.join(OUT,"ordered-reps-results.json"),JSON.stringify(result,null,2)+"\n");
 console.log("MASTER40_ORDERED_REPS "+JSON.stringify(result));
