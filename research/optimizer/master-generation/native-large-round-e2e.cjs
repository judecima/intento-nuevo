...
function shard(){
 ...
 fs.writeFileSync(path.join(OUT,"native-large-round-e2e-shard-"+SHARD_INDEX+".json"),JSON.stringify({shard:SHARD_INDEX,total:SHARD_TOTAL,meta:{selected:meta.selected.length,matchedCount:meta.matchedCount,masterCount:meta.masterCount,unmatchedCount:meta.unmatchedCount},rows},null,2)+"\n");
}
...
function report(){
 ...
 const files=fs.readdirSync(OUT).filter(x=>/^native-large-round-e2e-shard-\d+\.json$/.test(x)),payloads=files.map(f=>JSON.parse(fs.readFileSync(path.join(OUT,f),"utf8"))),rows=payloads.flatMap(x=>x.rows).sort((a,b)=>a.index-b.index);
 ...
 const result={schema:"master-40-native-large-round-e2e-v1",rounds:40,cohort:payloads[0].meta,
 ...
 fs.writeFileSync(path.join(OUT,"native-large-round-e2e-results.json"),JSON.stringify(result,null,2)+"\n");console.log("MASTER40_NATIVE_LARGE_ROUND_E2E "+JSON.stringify(result));
