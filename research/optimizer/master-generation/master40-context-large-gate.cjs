...
function child(file,candidate){
 ...
 if(candidate) process.env.OPTIMIZER_RUST_MASTER40_CONTEXT_EXPERIMENTAL="1";
 else delete process.env.OPTIMIZER_RUST_MASTER40_CONTEXT_EXPERIMENTAL;
 ...
 const row={file,pieces:target[1],mode:candidate?"candidate":"control",ms,poolSize:pool.length,digest:patternDigest(pool),rust:cfg._rustPatternGeneratorUsed||null,fallback:Boolean(cfg._rustPatternGeneratorFallback),roundPolicy:cfg._patternMaskPolicy||null,master40Context:cfg._rustMaster40Context||null};
...
function parent(){
 ...
 for(let i=0;i<TARGETS.length;i++){
  ...
  pair.contextEligible=Boolean(pair.candidate.master40Context?.eligible);
  rows.push(pair);
 ...
 const result={schema:"master-40-round-master40-context-large-v1",rounds:40,rows,summary:{controlMs:+control.toFixed(2),candidateMs:+candidate.toFixed(2),speedupPct:pct(control,candidate),poolParity:rows.every(r=>r.poolParity),eligibleCases:rows.filter(r=>r.contextEligible).length}};
 fs.mkdirSync(OUT,{recursive:true});fs.writeFileSync(path.join(OUT,"master40-context-large-results.json"),JSON.stringify(result,null,2)+"\n");
 console.log("MASTER40_RUST_CONTEXT_LARGE "+JSON.stringify(result));
