...
function child(file,pieces,candidate){
 ...
 delete process.env.OPTIMIZER_RUST_LARGE_ROUND_EXPERIMENTAL;
 if(candidate) process.env.OPTIMIZER_RUST_MASTER_LARGE_BATCH_EXPERIMENTAL="1";
 else delete process.env.OPTIMIZER_RUST_MASTER_LARGE_BATCH_EXPERIMENTAL;
 ...
 process.stdout.write("RESULT "+JSON.stringify({file,pieces,mode:candidate?"candidate":"control",ms,poolSize:pool.length,digest:poolDigest(pool),fallback:Boolean(config._rustPatternGeneratorFallback),masterContext:config._rustMasterContext||null})+"\n");
}
...
function shard(){
 ...
 fs.writeFileSync(path.join(OUT,"master-large-context-broad-shard-"+SHARD_INDEX+".json"),JSON.stringify({shard:SHARD_INDEX,total:SHARD_TOTAL,meta:{selected:meta.selected.length,matchedCount:meta.matchedCount,masterCount:meta.masterCount,unmatchedCount:meta.unmatchedCount},rows},null,2)+"\n");
}
...
function report(){
 const files=fs.readdirSync(OUT).filter(x=>/^master-large-context-broad-shard-\d+\.json$/.test(x)),payloads=files.map(f=>JSON.parse(fs.readFileSync(path.join(OUT,f),"utf8"))),rows=payloads.flatMap(x=>x.rows).sort((a,b)=>a.index-b.index);
