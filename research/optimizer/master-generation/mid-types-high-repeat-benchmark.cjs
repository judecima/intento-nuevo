...
function shard(){
 ...
 for(let i=SHARD_INDEX;i<manifest.rows.length;i+=SHARD_TOTAL){
   ...
   console.log("MID_REPEAT "+JSON.stringify({order:m.order,win:Number(m.masterWins||0)>0,pre,p3:p3.boards,full:full.boards,parity,p3Cpu:p3.totalCpuMs,fullCpu:full.totalCpuMs}));
 }
 fs.writeFileSync(path.join(__dirname,"mid-types-high-repeat-benchmark-shard-"+SHARD_INDEX+".json"),JSON.stringify({schema:"mid-types-high-repeat-benchmark-shard-v1",shard:SHARD_INDEX,shardTotal:SHARD_TOTAL,rows},null,2)+"\n");
}
...
function report(){
 const expected=Number(process.env.SHARD_TOTAL||16),files=fs.readdirSync(__dirname).filter(x=>/^mid-types-high-repeat-benchmark-shard-\d+\.json$/.test(x));
 ...
 const result={schema:"mid-types-high-repeat-p3-vs-full40-v1",cases:rows.length,wins:wins.length,nonWins:nonwins.length,parityFailures,
