...
function digest(plan,includeRef=true){const b=(plan?.placas??[]).map(x=>({p:(x.colocadas??[]).map(p=>includeRef?[p?.pieza?.ref,p.base,p.altura,p.x,p.y,Boolean(p.rotada),p.nivel??0]:[p.base,p.altura,p.x,p.y,Boolean(p.rotada),p.nivel??0]),c:(x.cortes??[]).map(c=>[c.x1,c.y1,c.x2,c.y2,c.nivel??0,Boolean(c.terminal)]),r:(x.restos??[]).map(r=>[r.x,r.y,r.w,r.h])}));return crypto.createHash("sha256").update(JSON.stringify(b)).digest("hex");}
function run(p,expected,mode){
 ...
 return{mode,ok:!err&&Boolean(v?.ok),boards:plan?.resumen?.placas??null,ms,digest:plan?digest(plan,true):null,geometryDigest:plan?digest(plan,false):null,cacheHits:plan?.resumen?.cacheHits??0,error:err,placements:plan?(plan.placas||[]).flatMap((b,bi)=>(b.colocadas||[]).map(x=>({board:bi,ref:x.pieza?.ref,id:x.pieza?.id,base:x.base,altura:x.altura,x:x.x,y:x.y,rotada:Boolean(x.rotada)}))):[]};
}
