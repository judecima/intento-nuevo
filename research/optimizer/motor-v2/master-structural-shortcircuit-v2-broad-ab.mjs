...
const q=(p,c)=>calidadPlanPlacas(p?.placas||[],p?.opts||c);
...
function summarize(rows){
 const same=rows.filter(r=>r.boardCmp===0),wins=rows.filter(r=>r.boardCmp<0);
 const rt=rows.reduce((s,r)=>s+r.routeMs,0),bt=rows.reduce((s,r)=>s+r.betaMs,0);
 return {
  cases:rows.length,
  structuralAttempted:rows.filter(r=>r.structuralAttempted).length,
...
if(MODE==="shard"){
 ...
 for(const c of cases){
  ...
  const a=timed(()=>runMasterStructuralShortCircuitV2(L,C));
  ...
  const boardCmp=cmp(ba,bb),qcmp=va&&vb&&boardCmp===0?compararCalidad(q(pa,C),q(pb,C)):null;
  const s=a.value?.structuralMaster||{};
  rows.push({
    ...
    structuralAttempted:Boolean(s.attempted),certified:Boolean(s.certified),shortCircuit:Boolean(s.shortCircuit),
    structuralBoards:s.boards??null,lb:s.lb??null,tests:s.tests??0,validMixed:s.validMixed??0,stopReason:s.stopReason??s.reason??null,
    structuralMs:s.ms??0,extraPatterns:s.extraPatterns??0,
    routeMs:a.ms,betaMs:b.ms,errorRoute:a.error,errorBeta:b.error
  });
 }
 fs.writeFileSync(path.join(HERE,`master-structural-v2-shard-${SI}.json`),JSON.stringify({shard:SI,total:ST,eligibleCases:eligible.length,rows})+"\n");
 console.log("MASTER_STRUCTURAL_SHARD "+JSON.stringify({shard:SI,cases:rows.length,eligible:eligible.length,summary:summarize(rows)}));
}else if(MODE==="report"){
 const rows=[];let eligibleCases=null;
 for(let i=0;i<ST;i++){const j=JSON.parse(fs.readFileSync(path.join(HERE,`master-structural-v2-shard-${i}.json`),"utf8"));eligibleCases=j.eligibleCases;rows.push(...j.rows);}
 rows.sort((a,b)=>a.id-b.id);
 const summary=summarize(rows);
 const failures=rows.filter(r=>!r.validRoute||!r.validBeta||r.boardCmp>0||(r.boardCmp===0&&r.qcmp<0));
 const wins=rows.filter(r=>r.boardCmp<0),shorts=rows.filter(r=>r.shortCircuit);
 const status=!failures.length&&summary.timing.savingPct>0?"PASS":"FAIL";
 const out={status,eligibleCases,summary,failures,wins,shortCircuits:shorts};
 fs.writeFileSync(path.join(HERE,"MASTER_STRUCTURAL_SHORTCIRCUIT_V2_RESULT.json"),JSON.stringify(out,null,2)+"\n");
 console.log("MASTER_STRUCTURAL_RESULT "+JSON.stringify({status,summary,wins:wins.length,shortCircuits:shorts.length,failures:failures.length}));
