...
for (const { name, context } of H2_FIXTURES) test(`B0.1 matches independent oracle: ${name}`, () => {
...
test("exact indices and suspendable linear coordinates preserve all legal candidates", () => {
  for (const { context } of H2_FIXTURES) {
    ...
    for (const axis of ["x", "y"]) for (const width of [context.width, ...context.types.map((t) => t.cutWidth)]) {
      ...
      assert.deepEqual(actual, [...cutCoordinates(context, state, createWorkBudget(H2_LIMITS), { coordinateStreams: 0, coordinateProposals: 0, duplicateCoordinates: 0 })]);
    }
  }
});

test("agenda interleaves axes and gives both propagation and state advancement a turn", () => {
  const agenda = createAgenda(["x", "y"]), actual = [];
  for (const axis of ["x", "y"]) for (const kind of [false, true]) agenda.add(axis, kind, () => {});
  for (let i = 0; i < 4; i++) { const task = agenda.take(); actual.push([task.axis, task.propagation]); }
  assert.deepEqual(actual, [["x", false], ["y", false], ["x", true], ["y", true]]);
});
