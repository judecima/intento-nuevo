...
import { generateIndustrialPortfolio } from "./portfolio.mjs";
...
if (!["portfolio", "legacy"].includes(mode)) throw new Error("mode must be portfolio|legacy");
const lines = [
...
if (mode === "portfolio") ({ patterns, telemetry } = generateIndustrialPortfolio(lines, config));
...
const coverage = resolverCobertura(ordered, lines.map((l) => l.cant), config.placaBase * config.placaAltura, 7, 20_000,
  { maxNodos: 1_600_000, watchdogMs: 60_000 }).resolver(lines.map((l) => l.base * l.altura));
const plan = coverage.plan ? materializar(coverage.plan, lines, config) : null;
const validation = plan ? validarPlanIndustrial(plan, 94) : null;
