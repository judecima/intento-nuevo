...
export async function enqueueOptimizationJob({
  ...
  if (active) {
    const row = active as Pick<JobRowWithProfile, "id" | "profile">;
    if ((row.profile ?? null) === (profile ?? null)) {
      await markProjectOptimizing(supabase, projectId, projectVersion);
      return { ok: true, jobId: row.id, reused: true };
    }

    return {
      ok: false,
      error: "OPTIMIZATION_ALREADY_RUNNING_WITH_DIFFERENT_PROFILE"
    };
  }
