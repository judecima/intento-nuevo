...
function optimizarV10(lineas, config, metricas = nuevasMetricas()) {
  ...
  // puede reducir placas. Conservar `mejor` tambien conserva exactamente el
  // remanente ya elegido: el Master legacy solo acepta candidatos con menos
  // placas y nunca reemplaza por empate de cantidad.
  if (
    ...
    if (cheapMaster > 0) {
      if (cheapMaster <= mejor.resumen.placas) {
        ...
        // Una LB no puede superar una solucion fisica factible.
