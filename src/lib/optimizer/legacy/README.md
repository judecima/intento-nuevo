# Legacy Optimizer

These CommonJS modules are mechanically extracted from `Optimizador_V10_Interactivo_Modos_Validacion_XML_Lepton.html` by `scripts/extract-legacy-optimizer.mjs`.

The guillotine optimizer is treated as critical legacy logic. Do not edit these generated files manually. Add typed adapters, validators, and tests around them instead.
