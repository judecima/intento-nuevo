...
function candBeats(sobra, a, area, exacta, target, criterio){
  if(criterio==='perp')  return sobra!==target.sobra ? sobra<target.sobra : (a!==target.a ? a>target.a : area>target.area);
  if(criterio==='area')  return area!==target.area ? area>target.area : sobra<target.sobra;
  if(criterio==='largo') return a!==target.a ? a>target.a : sobra<target.sobra;
  if(exacta!==target.exacta) return exacta<target.exacta;
  if(sobra!==target.sobra)   return sobra<target.sobra;
...
function elegir(pool, region, restante, perp, opts, rnd, nivel){
  ...
  for(let r=0;r<reps.length;r++){
    ...
    for(let k=0;k<ors.length;k++){
      ...
      const exacta = sobra < 1e-9 ? 0 : 1;
      const areaCand = a * b;

      if (!m3 || (opts.penalizarFranjaMuerta && nivel <= 2) || candBeats(sobra, a, areaCand, exacta, m3, criterio)) {
        considerar({
          i:p._pos, o, a, b, sobra,
          exacta,
          area:areaCand, mult:1, riesgoFranja
        });
      }
      ...
      if(opts.multiRebanada && nivel<opts.etapas && disp>=2){
        for(let mk=2; mk<=Math.min(3,disp); mk++){
          ...
          if(!sirve) continue;
          const areaT = t * b;
          if (!m3 || (opts.penalizarFranjaMuerta && nivel <= 2) || candBeats(sobra, t, areaT, 1, m3, criterio)) {
            considerar({i:p._pos, o, a:t, b, sobra, exacta:1, area:areaT, mult:mk});
          }
        }
      }
    }
...
function calidadRestos(restos,opts){
  let mayor=0, segundo=0, fragmentos=0, total=0;
  if(restos && restos.length){
    const minLado=opts.restoMin, maxLado=opts.restoMax;
    for(let i=0;i<restos.length;i++){
      const r=restos[i];
      const rw=r.w, rh=r.h;
      if((rw<rh?rw:rh)>=minLado && (rw>rh?rw:rh)>=maxLado){
        const a=rw*rh;
        total+=a;
        fragmentos++;
        if(a>mayor){
  ...
  return {mayor, segundo, fragmentos, total, areas:[mayor, segundo]};
}
...
function calidadPlanPlacas(placas,opts){
  ...
  if(placas && placas.length){
    ...
    for(let i=0;i<placas.length;i++){
      const restos=placas[i].restos;
      if(!restos) continue;
      for(let j=0;j<restos.length;j++){
        const r=restos[j];
        const rw=r.w, rh=r.h;
        if((rw<rh?rw:rh)>=minLado && (rw>rh?rw:rh)>=maxLado){
          const a=rw*rh;
