...
import { EXPERIMENTAL_STAGED_OPTIMIZER_VERSION, LEGACY_OPTIMIZER_VERSION } from "../engine/legacy-engine";
...
export function isEntryCompatible(entry: ExactExperienceEntry | null | undefined): boolean {
  ...
  if (
    entry.optimizerVersion !== LEGACY_OPTIMIZER_VERSION &&
    entry.optimizerVersion !== EXPERIMENTAL_STAGED_OPTIMIZER_VERSION
  ) {
    return false;
  }
