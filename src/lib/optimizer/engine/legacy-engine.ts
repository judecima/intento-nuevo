...
import { buildPieceKeys, revalidateExactHit } from "../experience/revalidate";
import { createExperienceEntry } from "../experience/store";
import type { ExactExperienceEntry } from "../experience/types";
...
const MAX_EXACT_GEOMETRIC_ENTRIES = 50;
const exactGeometricCache = new Map<string, ExactExperienceEntry>();
...
export function optimizeProject(input: OptimizationInput): OptimizationResult {
  ...
  const canonical = canonicalizeOptimizationInput(parsed);
  const geoFingerprint = exactFingerprint(canonical);
  const profile = parsed.constraints.profile ?? "balanced";
  const geoKey = `${geoFingerprint}|${strategy}|${profile}`;
  const geoEntry = exactGeometricCache.get(geoKey);

  if (geoEntry) {
    const reval = revalidateExactHit(geoEntry, parsed, canonical);
    if (reval.ok) {
      const hitResult = cloneOptimizationResult(reval.result);
      hitResult.metrics = { ...hitResult.metrics, cacheHit: true, engineMs: Date.now() - startedAt };
      rememberOptimizationResult(cacheKey, hitResult);
      return hitResult;
    }
  }
  ...
  if (strategy === "v10" && options.cotaInferiorExterna == null) {
    try {
      const { computeHybridLowerBound } = require("../experimental/hybrid-lower-bound.cjs");
      const hybrid = computeHybridLowerBound(lineas, options, undefined, { useRaster: false });
      const cheapLB = Math.max(0, Math.floor(Number(hybrid?.cheapLowerBound ?? hybrid?.lowerBound ?? 0)));
      if (cheapLB > 0) {
        options.cotaInferiorExterna = cheapLB;
      }
    } catch (_) {}
  }
  ...
  try {
    const pieceKeys = buildPieceKeys(result, parsed);
    if (pieceKeys && pieceKeys.length === result.placements.length) {
      if (exactGeometricCache.size >= MAX_EXACT_GEOMETRIC_ENTRIES) {
        const firstKey = exactGeometricCache.keys().next().value;
        if (firstKey) exactGeometricCache.delete(firstKey);
      }
      exactGeometricCache.set(
        geoKey,
        createExperienceEntry({
          exactFingerprint: geoFingerprint,
          strategy,
          profile: parsed.constraints.profile,
          plan: result,
          pieceKeys,
          originalEngineMs: Date.now() - startedAt
        })
      );
    }
  } catch (_) {}
