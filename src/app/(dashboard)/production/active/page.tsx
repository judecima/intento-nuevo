...
export default async function ProductionActivePage({ searchParams }: ProductionActivePageProps) {
  ...
  if (!organizationId || !canAccessProduction(context.role)) {
    return (
      <section className="max-w-6xl">
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">En produccion</h1>
  ...
  const [items, profiles] = organizationId
    ? await Promise.all([listProductionOrders(organizationId, ["production"]), listActiveMachineProfiles(organizationId)])
    : [[], []];
