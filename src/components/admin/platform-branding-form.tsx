  ...
  return (
    <form action={updatePlatformBrandingAction} className="space-y-4">
      <label className="block text-sm font-medium">
        Razon social / nombre del SaaS
        <input name="legalName" required minLength={2} defaultValue={legalName} className="input mt-2 w-full" />
      </label>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block text-sm font-medium">
          ...
          <input
            name="primaryColor"
            type="color"
            defaultValue={primaryColor || DEFAULT_PRIMARY_COLOR}
        ...
        <label className="block text-sm font-medium">
          ...
          <input
            name="secondaryColor"
            type="color"
            defaultValue={secondaryColor || DEFAULT_SECONDARY_COLOR}
            className="mt-2 h-11 w-full cursor-pointer border border-[var(--line)] bg-white p-1"
          />
        </label>
      </div>
      ...
      {logoUrl ? (
        <img src={logoUrl} alt="Logo actual del SaaS" className="h-14 max-w-[220px] object-contain object-left" />
      ) : (
        <p className="hint">Se utilizara el nombre del SaaS si no hay logo.</p>
      )}
      <PendingSubmitButton pendingLabel="Guardando identidad..." className="btn btn-primary focus-ring">
        Guardar identidad del SaaS
      </PendingSubmitButton>
    </form>
  );
}
