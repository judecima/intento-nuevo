...
## COTA INFERIOR FUERTE

Fecha: 2026-09-04
Commit base: 3eb1c97
Naturaleza: MODULO NUEVO AISLADO. No se conecto a produccion. No se modifico motor.cjs,
v10.cjs, patrones.cjs, cobertura.cjs, oneboard.cjs, materializar.cjs, legacy-engine.ts,
ningun default, ningun perfil ni ninguna heuristica. Con el modulo sin llamar, el
comportamiento del optimizador es identico al de antes, bit a bit.

### Motivacion

Sale directo de "HALLAZGOS DIAGNOSTICO COLA CARA V10". El separador del costo no es
estructural sino binario: alcanzar o no la cota inferior por area. 1680 casos la alcanzan y
consumen el 12.4% del tiempo; 320 no la alcanzan y consumen el 87.6%. La familia dominante
es F1: 222 casos que terminan a UNA sola placa de la cota, 41.6% del tiempo del holdout.

La cota vigente es la mas debil que existe para 2D-BPP:

    cota = ceil( SUM area_i / (anchoUtil * altoUtil) )

Ignora el kerf, la geometria y la restriccion guillotina. En buena parte de esos 320 casos
el plan de V8 ya es optimo y el motor no puede demostrarlo, asi que gasta 23.6 s de promedio
persiguiendo un objetivo inalcanzable. Esto atiende el pendiente registrado como "queda
abierta la pregunta de la cota inferior sin kerf".

A diferencia de recortar rondasPatrones, esta via no cambia ninguna decision: si la cota es
valida y el plan la alcanza, el plan es optimo por teorema y saltear los rescates no puede
costar una placa. El riesgo de regresion no es bajo, es nulo, condicionado a que la cota sea
valida. Por eso todo el peso de la validacion esta en demostrar validez.

### Cotas implementadas

Se calcula el maximo de cuatro cotas, todas O(n) u O(W):

1. AREA. La vigente. Se conserva para que `best` nunca quede por debajo del calculo actual.

2. INFLADO POR KERF. En cualquier plan factible con sierra s, dos piezas adyacentes estan
   separadas por al menos s sobre el eje del corte que las separa. Asignando a cada pieza el
   rectangulo [x, x+w+s] x [y, y+h+s], esos rectangulos son disjuntos y caben en
   [0, W+s] x [0, H+s]. Luego N >= ceil( SUM (w+s)(h+s) / ((W+s)(H+s)) ).
   Con s=4.5 y piezas de 400x300 son ~2.4% mas de area por pieza.

3. CRIBA DE REPRESENTABILIDAD. Por Herz (1972) y Christofides-Whitlock (1977) siempre existe
   un optimo guillotina en posiciones normales, asi que el largo consumido sobre cada eje
   pertenece al semigrupo numerico generado por {d_j + s}. Sea r* el maximo alcanzable <= W.
   Ninguna placa puede consumir mas de r* sobre X, asi que el area utilizable por placa es a
   lo sumo r* x H, no W x H. Se calcula con una criba booleana sobre enteros en decimas de
   milimetro y descomposicion binaria de multiplicidades.
   Si alguna medida no es representable exacta en la escala, la criba NO corre: se prefiere
   no aportar cota antes que aportar una invalida por redondeo.
   Direcciones seguras: AGRANDAR el conjunto alcanzable es valido (r* sube, la cota baja);
   ACHICARLO seria invalido, asi que nunca se descarta un modulo. Si hay demasiados, se
   saltea la criba entera.

4. FUNCIONES DUAL-FACTIBLES. Familia clasica u_e con e en (0, 1/2]:
       u_e(x) = 1 si x > 1-e ; x si e <= x <= 1-e ; 0 si x < e
   Es dual-factible: si SUM x_i <= 1, a lo sumo un item supera 1-e; sin item grande
   SUM u = SUM_{x>=e} x <= 1, y con item grande el resto suma menos que e y vale 0.
   Con una DFF por eje, N >= ceil( SUM u_e(w_i) u_d(h_i) ) sobre medidas normalizadas.
   Se aplica sobre las dimensiones infladas por kerf y sobre la placa ya reducida por la
   criba, asi que esta cota DOMINA a las tres anteriores: con e = d = 0 la funcion es la
   identidad y se recupera la cota de area con kerf.
   Con rotacion libre se toma el MINIMO sobre orientaciones, porque la orientacion la elige
   el solver y la relajacion tiene que ser adversarial.
   Solo importan los puntos de quiebre; se los submuestrea de forma deterministica a 24 por
   eje. Usar un subconjunto de la familia da una cota mas debil, nunca invalida.

### Archivos agregados

- src/lib/optimizer/bounds/lower-bounds.ts
- src/lib/optimizer/bounds/index.ts
- tests/optimizer/lower-bounds.test.ts
- scripts/lower-bound-ablation.mjs
- src/lib/optimizer/index.ts: se agregaron cuatro exports. No se modifico ningun export previo.

### Pruebas

- npx vitest run tests/optimizer/lower-bounds.test.ts: 13 tests passed.
- npx vitest run tests/optimizer tests/optimizations tests/production/xml.test.ts tests/domain/optimizations.test.ts: 12 archivos, 180 tests passed.
- npx tsc --noEmit: passed.
- npx eslint sobre los archivos nuevos con --max-warnings=0: passed.

El test que importa es el de validez: contrasta la cota contra planes FACTIBLES producidos
por motor.cjs sobre instancias aleatorias deterministas, 60 con rotacion libre y 30 con veta
y rotacion restringida. Una cota invalida convierte la compuerta en una regresion silenciosa,
asi que ese test es el gate del modulo.

### Fuzz de validez fuera del repo

Corrida de estres deterministica, 1500 instancias sobre 4 formatos de placa (2750x1830,
1200x800, 1830x2600, 3050x1300), 5 valores de kerf (0, 3, 4.5, 5, 8) y 30% de casos con veta.
1412 instancias factibles corridas contra motor.cjs.

- violaciones de validez: 0
- casos donde la cota fuerte supera a la de area: 782 (55.4%)
- placas de cota adicionales en total: 1789
- cota activa: area 630, kerf 18, raster 138, dff 626

Segunda corrida sobre medidas tipicas de mueble (alturas de modulo y profundidades reales,
8 a 19 tipos por pedido, placa 2750x1830 con refilado 10), 120 casos con el motor completo:

- fuera de la cota de area: 19 casos (15.8%, contra 16.0% real del holdout)
- cerrados por la cota fuerte: 6 de 19 (31.6%)
- violaciones: 0

Limitacion explicita: las dos corridas son sobre instancias SINTETICAS, no sobre el corpus
D:\proyectos asistidos\lepton\data\lepton-xml. El 31.6% es una estimacion, no una medicion.
El numero real hay que sacarlo con scripts/lower-bound-ablation.mjs.

### Ampliacion: proyeccion y clique (2026-09-04, segunda tanda)

Se agregaron al modulo dos cotas mas, tomadas de la descripcion del Strong LB del informe
V13, para poder medir las dos familias en el mismo banco:

5. PROYECCION. Si toda orientacion admisible de una pieza tiene alto mayor que H/2,
   la pieza cruza la recta media y = H/2 en cualquier ubicacion. Dos piezas asi, en la
   misma placa, tienen intervalos disjuntos sobre X separados por un kerf. Luego
   SUM (w_i + s) <= W + s por placa, y N >= ceil( SUM (w_i + s) / (W + s) ). Se toma el
   minimo ancho entre orientaciones que califican. Simetrica sobre el otro eje.

6. CLIQUE DE INCOMPATIBILIDAD. Dos rectangulos caben juntos si y solo si existe un par de
   orientaciones con a1+s+a2 <= W y max(b1,b2) <= H, o b1+s+b2 <= H y max(a1,a2) <= W.
   El test es exacto para dos piezas porque cualquier empaque de dos rectangulos se
   normaliza a lado a lado o apilado. Un conjunto incompatible de a pares necesita una
   placa por pieza, asi que cualquier clique es una cota valida. Se busca golosa con dos
   ordenes distintos y se toma el mejor. Trabaja a nivel de TIPO: un tipo incompatible
   consigo mismo aporta sus `cant` unidades.

La cardinalidad de piezas mayores a la mitad queda subsumida por el clique: dos piezas que
superan la mitad en ambos ejes son incompatibles por definicion.

Clique y DFF NO se dominan mutuamente. La DFF acumula fracciones que el clique no ve; el
clique captura conflictos geometricos que ninguna funcion separable por eje puede expresar.
Por eso se calculan las dos y se toma el maximo.

Fuzz de validez repetido con las 6 cotas, mismas 1412 instancias: 0 violaciones.
Cota activa: dff 625, area 630, raster 138, kerf 18, clique 1, proyeccion 0.

### Ablacion por familia (datos sinteticos de mueble)

100 pedidos de 8 a 19 tipos con medidas tipicas de mueble, placa 2750x1830 refilado 10,
kerf 4.5, motor completo para obtener las placas.

    fuera de la cota de area = 18    violaciones = 0
    certifica (area + proyeccion + clique)          = 0
    certifica (area + kerf + dff + criba)           = 6
    ambos = 0    solo el primero = 0    solo el segundo = 6
    union = 6
    sin criba raster = 5    sin criba y con dff 8x8 = 5

LIMITACION IMPORTANTE: sobre el corpus REAL el Strong LB del informe V13 certifica 28 de
320 casos, o sea que la familia proyeccion/clique si aporta ahi. Que aca de 0 es una
limitacion del generador sintetico, que produce pocas piezas que superen la mitad de la
placa en ambos ejes. NO se puede concluir que esa familia sea inutil. Lo unico que este
banco establece es que las dos familias son disjuntas en esta poblacion y que ninguna
domina a la otra, que es lo que ya decia el argumento teorico.

La medicion que decide es la ablacion equivalente sobre el corpus real.

### Costo por componente

300 pedidos de 8 a 19 tipos, medido en aislamiento con warmup:

| configuracion                  | ms por caso |
|--------------------------------|------------:|
| solo area                      |      0.0049 |
| + kerf                         |      0.0067 |
| + proyeccion                   |      0.0326 |
...
| + criba raster                 |      1.3783 |
| todas                          |      1.3740 |
| todas menos criba              |      0.2231 |
| todas menos criba, dff 8x8     |      0.0830 |

La criba raster domina el costo: es 6 veces todo lo demas junto, porque recorre un array de
decimas de milimetro del ancho de placa. Y en la ablacion aporta 1 certificado de 6.

Consecuencia operativa: la criba se justifica en balanced/v10, donde 1.4 ms contra 23.6 s de
promedio es nada. En el perfil fast, cuyo p50 es 11 ms, conviene apagarla y bajar la DFF a
8x8: 0.083 ms por caso, del mismo orden que el Strong LB del informe V13 (0.1125 ms), y
conserva 5 de los 6 certificados.

### Lectura del gate staged V14 (50 casos) - problemas de muestreo

Analisis de v14_staged_historical_50.json y v14_incremental_physical_equivalence_51.json.
Ninguno de los dos mide lo que hay que medir. Los hallazgos, en orden de gravedad:

1. LA MUESTRA ES LA COLA BARATA. Los 50 casos tienen p50 = 9.5 piezas y 5 tipos; 32 de 50
   tienen 12 piezas o menos y solo 1 llega a 30. La familia F1, que explica el 41.6% del
   tiempo del holdout, tiene mediana de 36 piezas y 13 tipos. El filtro de seleccion fue
   histGen20 moderado, que por construccion elige pedidos chicos. El gate mide justo la
   poblacion que no cuesta.

2. NO SE PUEDE ATRIBUIR NINGUNO DE LOS 15 CERTIFICADOS. Las filas traen strongLB pero no
   traen la cota de area del mismo caso. Los 15 certificados son exactamente los 15 casos
   con gap 0, pero sin la columna de area no se sabe si el Strong LB aporto algo o si esos
   casos ya estaban en cota de area y V10 legacy tambien habria retornado temprano. En ese
   escenario los 15 certificados valen cero. FALTA LA COLUMNA areaLB.

3. LOS 35 RESTANTES SON TODOS GAP +1, SIN EXCEPCION. La distribucion de A40 - strongLB es
   15 casos en 0 y 35 casos en 1. O sea que en esta muestra el Strong LB no cerro ni un
   solo caso por encima de la cota de area. Esos 35 son la poblacion objetivo de las cotas
   por kerf, DFF y criba, que atacan precisamente el margen de una placa.

4. NO HAY MEDICION DE VELOCIDAD. El archivo trae ms del staged pero ninguna columna A del
   mismo proceso. El unico dato de velocidad del informe es un caso (4048053, 1734 -> 383 ms).
   El gate demuestra correccion (50/50 mismas placas) y no dice nada sobre el objetivo, que
   es tiempo. Ademas los tiempos absolutos preocupan: p50 de 3475 ms en pedidos de 9.5 piezas,
   y 1683 ms en uno de 4 piezas.

5. EL FALLBACK SE COME EL 87.6% DEL TIEMPO DE LA TANDA (177918 de 203087 ms), la misma
   proporcion que los casos fuera de cota en el holdout. La estructura se repite a otra
   escala: los gap +1 son todo el costo.

6. LA AUDITORIA DE EQUIVALENCIA EXCLUYE POR CONSTRUCCION LOS CASOS DONDE LA EQUIVALENCIA
   PUEDE ROMPERSE. El set de 51 se define como "first 51 deterministic historical cases with
   generarPatrones <= 2000 ms". Con 20 rondas en menos de 2000 ms, cada ronda usa ~100 ms y
   el beam nunca toca su techo de presupuestoBeamMs = 1500 ms. La divergencia entre
   monolitico e incremental solo puede aparecer cuando el presupuesto de reloj BINDEA, que es
   lo que pasa en la cola cara. El 51/51 es correcto y no prueba lo que se le pide.

7. El caso 4051690 esta en el set de equivalencia de 51 aunque fue excluido en V13 por
   inconsistencia de corpus. Es defendible, porque la equivalencia compara staged contra
   monolitico sobre el mismo input canonico y no contra el historico, pero conviene anotarlo
   para que nadie cite el 51/51 como evidencia de consistencia historica.

8. El control de "subflags apagadas reproduce V10" se hizo sobre 1 caso (4048053). Es el
   contrato del feature flag y deberia ser un test sobre los 50, no una observacion manual.

### Como sacar los numeros faltantes

El script scripts/lower-bound-ablation.mjs acepta --orders y ya registra por caso las seis
cotas por separado (area, kerf, raster, dff, proyeccion, clique), el maximo y cual queda
activa. Eso da directamente la columna areaLB del punto 2 y la atribucion del punto 3:

    node scripts/lower-bound-ablation.mjs bounds --orders experiencia/v14_staged_historical_50.json
    node scripts/lower-bound-ablation.mjs verify --orders experiencia/v14_staged_historical_50.json

El reporte imprime una tabla de atribucion con, por cada termino, cuantos casos alcanza por
si solo y en cuantos es el unico que cierra. La corrida `bounds` no ejecuta el motor y
termina en segundos.

### Validacion externa con oracle exacto (informe de pruebas, 2026-09-04)

Se construyo un solver exacto independiente para packing guillotina pequeno (enumeracion de
composiciones slicing por subconjunto, dimensiones Pareto, DP de particion minima) y se
contrastaron las seis cotas contra el OPTIMO EXACTO, no contra un plan factible del motor.

    tanda A:  3000 instancias, 2-8 piezas, kerf entero, rotacion libre y bloqueada
    tanda B: 10000 instancias, medidas en incrementos de 0.5, kerf fraccionario, repeticiones
    total: 13000 instancias exactas, 0 violaciones

Esto cierra el hueco que tenia la auditoria anterior. Comparar contra una solucion V10-40
solo detecta cotas que superan un valor ALCANZABLE; el falso certificado ocurre cuando la
cota iguala las placas del plan pero el optimo verdadero es menor, y ahi V10-40 tampoco
encuentra nada mejor. El oracle exacto si detecta ese caso.

Limitacion: el oracle cubre 2-8 piezas, asi que valida el algebra de las cotas pero no los
caminos de n grande de la implementacion. Esos caminos son seguros por construccion y
conviene dejarlo anotado:
- el submuestreo de candidatos DFF (maxCandidatosDff) usa un SUBCONJUNTO de una familia
  valida, lo que da una cota mas debil y nunca invalida;
- los topes de la criba (maxCeldasRaster, maxModulosRaster) y el guardia de escala entera
  devuelven `fallback`, o sea sin reduccion de placa, que es el caso neutro.

### Ablacion sobre el holdout historico consistente

1826 casos historicos donde la cantidad de piezas reconstruida coincide con V10-20 y V10-40.
Familia F1 definida como boards40 = areaLB + 1: 231 casos. Familia F2: 2 casos.

| cota       | unsafe vs V10-40 | eleva area | certifica F1 | costo medio |
|------------|-----------------:|-----------:|-------------:|------------:|
| area       |                0 |          0 |        0/231 |   0.0028 ms |
| kerf       |                0 |         31 |       28/231 |   0.0015 ms |
| dff        |                0 |         70 |       64/231 |   0.2905 ms |
| raster     |                0 |         47 |       43/231 |   1.2499 ms |
| proyeccion |                0 |         51 |       47/231 |   0.0039 ms |
| clique     |                0 |         44 |       41/231 |   0.0304 ms |
| todas      |                0 |         76 |       70/231 |   1.5640 ms |

Contra el Strong LB V14 sobre los mismos 1826 casos: V14 certifica 1615, las seis cotas
juntas certifican 1658. En F1: V14 28/231, las seis 70/231. Los casos que gana el conjunto
nuevo concentran ~576.9 s de generarPatrones historico.

### INCONSISTENCIA ABIERTA en esos numeros

Dos cosas no cierran y hay que resolverlas antes de descartar proyeccion y clique:

1. El Strong LB V14 se describe como area + proyeccion + cardinalidad de piezas mayores a la
   mitad + clique. Si eso es asi, V14 tiene que certificar al menos tanto como el mejor de
   sus componentes, o sea >= max(47, 41) = 47 casos F1. El informe le atribuye 28. O la
   implementacion V14 de proyeccion/clique es mas debil que la de este modulo, o el 28 es un
   numero arrastrado de una medicion sobre otro conjunto (los 320 fuera de cota del holdout
   de 2000, que no es el mismo universo que los 231 F1 de los 1826 consistentes).

   Consecuencia: la conclusion "no priorizaria proyeccion/clique porque V14 ya cubre gran
   parte de esa geometria" NO esta sostenida por la tabla del propio informe. Hay que
   re-medir V14 sobre los 1826 antes de decidir eso.

2. 70 - 28 = 42, pero el informe reporta "+43 casos F1". Global: 1658 - 1615 = 43, pero
   reporta "+44 casos adicionales". El mismo desfasaje de 1 en los dos lados implica que
   |Claude \ V14| = 43 y |V14 \ Claude| = 1: existe AL MENOS UN caso F1 que V14 certifica y
   las seis cotas de este modulo no.

   Ese caso es diagnostico. Aislarlo dice que tiene V14 que a este modulo le falta. Y mientras
   exista, la decision correcta no es reemplazar V14 sino unir: max(V14, cascada).

### Cascada en dos etapas: computeLowerBoundCascade

Implementada en lower-bounds.ts a partir de la arquitectura propuesta en el informe.

    etapa barata: area + kerf + dff + proyeccion + clique      (sin criba)
    si certifica el incumbente -> terminar
    etapa completa: agrega la criba raster

Justificacion con los numeros de arriba: la DFF explica 64 de los 70 certificados y la criba
cuesta 4 veces todo lo demas junto. Corriendo la criba solo cuando la etapa barata no
certifica, se ejecuta en 173 de 1826 casos, agrega 6 certificados y cuesta ~2.41 ms cuando
corre. Son 0.42 s de costo contra 16.2 s de generarPatrones evitado: 39x de retorno.

Ese retorno de 39x es mejor de lo que sugiere la lectura del informe, que presenta la criba
como marginal. Es marginal EN EL CAMINO RAPIDO y muy rentable como segunda etapa.

Firma: computeLowerBoundCascade(lineas, opts, incumbente?, config?). Sin incumbente corre las
dos etapas, que es lo correcto cuando se quiere la cota mas ajustada y no una decision de
compuerta. Devuelve `etapa`, `rasterCorrio` y `bestBarata` para poder registrar la atribucion
por caso.

### Sobre 4058501

Las seis cotas devuelven 7 y el plan es 8, asi que este modulo tampoco lo certifica. Es el
resultado esperado y no cambia la recomendacion previa: no perseguir ese caso. Ninguna cota
separable por eje ni ninguna relajacion de asignacion va a cerrarlo; la unica relajacion
barata lo bastante fuerte seria la cota LP de Gilmore-Gomory por generacion de columnas, y
eso se justifica por reemplazar el sorteo de patrones, no por un caso.

### Oracle exacto dentro del repo (2026-09-04, tercera tanda)

Se incorporo un oracle guillotina exacto a tests/optimizer/lower-bounds.test.ts. Motivo: el
oracle externo prueba la version auditada del modulo; este prueba la version que el commit
tiene en disco y corre en CI cada vez que alguien toca lower-bounds.ts. Sin eso, dentro de
seis meses un cambio al modulo no tiene guardia dentro del repo.

Metodo: para cada subconjunto se calcula el conjunto Pareto-minimal de bounding boxes
alcanzables por un arbol guillotina, combinando subconjuntos de a pares (lado a lado sumando
kerf, o apilados sumando kerf). Un subconjunto entra en una placa si alguna de sus cajas cabe.
Luego DP sobre subconjuntos para la particion minima. Exponencial, limitado a 6 piezas.

Dos tandas deterministas de 700 instancias, una con medidas enteras y otra con medidas y kerf
fraccionarios, veta y rotacion mezcladas. Verifica computeLowerBound y computeLowerBoundCascade
contra el optimo EXACTO, no contra un plan factible.

Se renombro el bloque anterior a "REGRESION CONTRA EL MOTOR REAL" y se corrigio su comentario.
La observacion es correcta: contrastar contra un plan factible solo detecta una cota que supera
un valor ALCANZABLE, y el modo de falla que importa es el otro. El test se conserva igual
porque cubre instancias mucho mas grandes que las que el oracle puede resolver.

### Mutation testing del oracle

Para verificar que el oracle tiene dientes se rompieron tres cosas a proposito:

1. Inflado de kerf al doble ((w+2s)(h+2s)). Detectado, con contraejemplo:
   optimo=1, raster=2, placa 616x302, s=3, dos piezas.
2. Clique contando multiplicidad siempre, sin exigir que el tipo choque consigo mismo.
   Detectado.
3. DFF admitiendo epsilon mayor a 1/2, que rompe la demostracion de dual-factibilidad.
   Detectado, con contraejemplo: optimo=3, dff=5, placa 766x393, s=5, seis piezas.

Las tres mutaciones fueron revertidas. Suite completa: 189 tests.

### Arquitectura acordada con el informe V2

    V10 pre-Master
      -> cheapLB = max( strongLB_V14 , cascada barata sin raster )
      -> si certifica: terminar
      -> raster condicional, volver a tomar el maximo
      -> si certifica: terminar
      -> 20 rondas -> Integrality Repair -> incremental 21..40

La union con V14 en vez del reemplazo es la decision correcta y coincide con lo que ya se
habia registrado: mientras exista al menos un caso F1 que V14 certifica y este modulo no, hay
que unir. Cuesta 0.03 ms mantener las dos.

### Inconsistencias que siguen abiertas

Ninguna de las dos registradas en la seccion anterior fue resuelta por el informe V2. Usar
`max` las esquiva sin responderlas, lo cual es seguro como decision de ingenieria pero deja
un numero mal en algun lado:

1. Sigue reportandose V14 = 28/231 mientras proyeccion sola da 47/231 y clique sola 41/231.
   Si V14 contiene proyeccion y clique, no puede certificar 28. O la implementacion V14 de
   esos dos terminos tiene un bug, y en ese caso el bug puede estar afectando otras cosas, o
   el 28 viene de un universo distinto de casos.

2. El total de la cascada se movio entre informes: 1658 en el primero, 1659 en el segundo,
   para el mismo calculo sobre los mismos 1826 casos. Este modulo es deterministico y tiene
   test que lo verifica, asi que el desvio viene del harness. Es el mismo problema de
   reproducibilidad de los presupuestos de reloj, apareciendo ahora en la medicion de las
   cotas.

### Benchmark F1 pesado V15: donde vive el valor de cada cota

F1 pesado definido como boards40 = areaLB + 1 con piece_count >= 20 y types >= 8.

    top 50 F1 mas caros (ranking por generarPatrones historico de 20 rondas)
      Strong LB V14      0 / 50
      hibrido barato     7 / 50
      hibrido + raster   7 / 50      (raster: +0)
      boards20 == boards40: 50/50
      unsafe: 0

    F1 pesado completo (156 casos)
      Strong LB V14       4 / 156
      hibrido barato     31 / 156
      hibrido + raster   32 / 156    (raster: +1)

De aca sale la particion del universo F1, que es el hallazgo principal:

    F1 total 231 = 156 pesados + 75 livianos
    V14 certifica 28 en total, de los cuales 4 son pesados => 24 livianos
      tasa en pesados  =  2.6%
      tasa en livianos = 32.0%

El Strong LB V14 concentra su capacidad en la mitad del universo que no cuesta. El hibrido
barato es 7.8x mejor en la cola pesada. Medido en generacion historica de los 156 pesados
(3181 s en total), V14 captura 36.3 s (1.1%) y el hibrido 586.2 s (18.4%).

Esto CIERRA la inconsistencia registrada antes (V14 = 28/231 contra proyeccion sola = 47/231).
La discrepancia sigue sin explicarse a nivel de implementacion, pero deja de tener consecuencia
practica: la arquitectura acordada usa max(V14, cascada) con la proyeccion y el clique de este
modulo, asi que si la version V14 de esos terminos tiene un bug, no muerde. Se baja de
prioridad; no se cierra formalmente.

### Por que el kerf domina en la cola pesada

Es el resultado mas util del benchmark y tiene explicacion mecanica. La penalizacion del
inflado por kerf es n*s*(w+h) aproximadamente, o sea que escala con el PERIMETRO total, no con
el area. A area total constante repartida en n piezas:

    n piezas   medida      area total   penalizacion   % sobre area
          20   1579x1579     49.86 m2      0.285 m2         0.57%
          40   1117x1117     49.91 m2      0.403 m2         0.81%
          80    790x790      49.93 m2      0.570 m2         1.14%
         160    558x558      49.82 m2      0.807 m2         1.62%
         320    395x395      49.93 m2      1.144 m2         2.29%
         640    279x279      49.82 m2      1.620 m2         3.25%

Crece como raiz de n. Por eso el kerf dispara en pedidos de muchas piezas medianas y no en
pedidos de pocos paneles grandes. Y es el espejo exacto de la DFF y el clique, que necesitan
piezas GRANDES respecto de la placa y por eso viven en la cola liviana.

Consecuencia: la cota mas barata de todas (0.0015 ms, la mas barata del modulo) es la que mas
rinde en la cola cara. Confirma sobre datos reales el argumento inicial de que un margen de
~2.4% de area por pieza es justo del tamano de la placa marginal que separa gap+1 de gap 0.

### Politica de tamano para la criba, ahora dentro del modulo

Medido: en el top 50 pesado la criba agrega 0 certificados y triplica el costo (3.19 -> 9.36 ms
promedio). Sobre los 156 pesados agrega 1 certificado y multiplica el costo por 4.7
(0.94 -> 4.43 ms), o sea ~544 ms de computo repartido para ganar un caso.

Se movio la politica adentro de computeLowerBoundCascade en vez de dejarla en el pipeline:

    rasterMaxPiezas: 40      (default)
    rasterMaxTipos:  16      (default)

Si el pedido excede cualquiera de los dos, la cascada devuelve la etapa barata y marca
`rasterOmitidoPorPolitica: true` para poder distinguirlo de "la barata ya certifico". Ambos
valores son configurables. Vive en el modulo para que no se pierda cuando alguien reescriba
el pipeline.

### Dos observaciones sobre las mediciones de la seccion 8

1. La cota reportada para 4057094 y 4050838 fue de 28 y 32 ms, contra un promedio medido de
   0.94 ms y un p95 de 2.50 ms en la misma cohorte pesada. Es un factor 30 sobre el promedio.
   Con la politica de tamano la criba no corre en esos pedidos (98 y 85 piezas), asi que no
   puede ser eso. Lo mas probable es JIT frio en la primera invocacion. Medir con warmup antes
   de publicar el costo, o el numero queda inflado 30x.

2. Esos dos casos siguen tardando ~23 s, de los cuales ~22.96 s son pre-master. El certificado
   evita Pattern Master entero, que es una ganancia real y grande (legacy no termino en 180 s),
   pero NO hace rapido al caso. El piso es el pre-master. Conviene decirlo asi en el informe
   para no crear la expectativa de que la cota resuelve la cola cara.

### Pregunta que abre el dato boards20 == boards40 en 50/50

En los 50 pedidos F1 mas caros, las rondas 21..40 no cambiaron la respuesta ni una vez. Se
suma a la ablacion previa, donde el master gano 1 placa en 129 activaciones sobre la cola cara
y 5 placas en 2000 pedidos.

Eso sugiere que el fallback 21..40 no compra nada en la cola pesada, que es exactamente donde
mas cuesta. Si sus victorias historicas estan en pedidos livianos, una regla del tipo "no
correr 21..40 cuando piezas >= 20 y tipos >= 8" seria casi gratis y ahorraria mucho mas que
hacer ese fallback incremental.

Es cambio de heuristica y requiere autorizacion explicita. Pero es medible con datos que ya
existen: ubicar en que casos del holdout las rondas 21..40 cambiaron el resultado, y ver si
alguno es pesado. Si ninguno lo es, la regla se justifica sola.

### Refutacion de la regla "no correr 21..40 en F1 pesado"

Se cruzo V10-20 contra V10-40 en el holdout historico. Existe UNA sola mejora de placas
entre 20 y 40 rondas en todo el holdout: 4058501, con 50 piezas, 11 tipos, areaLB 7,
20 rondas 9 placas, 40 rondas 8 placas. Por definicion es F1 pesado.

La regla propuesta ("si F1 pesado, no ejecutar 21..40") produciria una regresion real ahi.
RECHAZADA correctamente. La hipotesis en su forma ingenua queda cerrada.

Dos cosas que NO quedan cerradas por ese contraejemplo:

1. LA FORMA REFINADA SOBREVIVE. Segun el informe V13, Integrality Repair construye una
   solucion valida de 8 placas para 4058501 a partir del pool temprano, sin las columnas de
   las rondas 21..40, con validador_industrial_v3.ok = true. Entonces la regla

       "no correr 21..40 cuando Integrality Repair ya mejoro el incumbente"

   no esta refutada por 4058501: esta CONFIRMADA por el, porque Repair llega a las mismas 8
   placas. Lo que hay que medir es si Repair es confiable en general cuando dispara, y hoy
   eso se sabe de un solo caso. Es una hipotesis para una medicion, no una recomendacion.

2. EL PRECIO DEL FALLBACK NO ESTA EN NINGUN INFORME. Con las cifras del propio benchmark V15:

       cohorte F1 pesada: 156 casos, 3181 s de generarPatrones en rondas 0..19
       certifica el hibrido: 31  =>  corren el fallback: 125
       costo estimado del fallback en esa cohorte: ~2549 s
       (las rondas 20..39 cuestan aproximadamente lo mismo que 0..19)

       compra: 1 placa, en 1 pedido de todo el holdout

   No es un argumento para sacarlo: la regla de cero regresiones se sostiene y el corpus
   completo son 8669 pedidos, no 2000. Es un argumento para que el precio este a la vista
   cuando se decida. Medicion pendiente: cruzar V10-20 contra V10-40 sobre los 8669 y ver
   si el conteo de mejoras sigue siendo del orden de 1.

### Modo `cohort`: congelar la cohorte y detectar deriva

Los conteos se movieron entre informes para la misma definicion sobre el mismo corpus:

    casos consistentes:    1826  ->  1789
    F1 pesados:             156  ->   153
    total de la cascada:   1658  ->  1659

Mientras la cohorte no sea estable, dos mediciones tomadas en corridas distintas no son
comparables. "V14 certifica 28 de 231" y "el hibrido certifica 70 de 231" pueden no estar
hablando de los mismos 231 casos.

Se agrego a scripts/lower-bound-ablation.mjs:

    node scripts/lower-bound-ablation.mjs cohort --hist benchmark_project_v10.csv

Deriva la cohorte de casos consistentes (demanda canonica igual a la del historico, que es
la regla ya registrada tras el caso 4051690), la escribe con un hash, y en corridas
posteriores verifica que se re-derive identica. Si cambia, imprime que entro y que salio.

Ademas reporta la DIRECCION de las discrepancias, que era el numero pedido desde el analisis
del gate V14 y nunca se produjo. Un desbalance fuerte no es ruido de corpus: el atributo num
del panel aparece en ~4.9% de los paneles y el benchmark HTML no lo aplica, lo que produce
historico MENOR que canonico; discrepancias en el otro sentido son un mecanismo distinto y
no documentado (4051690 va en ese sentido).

Probado de punta a punta con un corpus sintetico: congela, detecta ausencia de deriva por
hash, y detecta deriva listando altas y bajas.

### Sobre la autoevaluacion V17

Coincidencias, sin repetir lo que ya esta bien:

- Preservar Compactacion pese al costo, por sus 273 mejoras historicas de remanente, y
  descartar la criba que solo ahorraba 7 ejecuciones, es exactamente el comportamiento
  correcto bajo el orden lexicografico placas > remanente > runtime.
- La metrica "Hybrid LB evita Master en 71 de los 241 casos donde Master tenia trabajo"
  es mucho mejor que las metricas de informes anteriores, porque el denominador es la
  poblacion donde la intervencion puede actuar.
- La frontera de no seguir agregando familias de DFF, fractales ni mutaciones exoticas es
  la correcta.

Reservas:

1. "2000/2000 mismas placas, 0 regresiones" NO ES VERIFICABLE hoy. presupuestoBeamMs y
   msMaster son presupuestos de reloj. El diagnostico del 2026-09-04 registro el caso
   4059200 dando 17 y 18 placas con el mismo codigo, solo por carga de maquina. Cualquier
   afirmacion de cero regresiones por re-ejecucion arrastra ese ruido. Es el unico
   prerrequisito que sigue abierto desde el principio.

2. La nueva regla propia, "toda mejora nueva tiene que demostrar que reduce tiempo sin
   cambiar placas ni remanente", es buena y tiene un agujero: no se puede demostrar sobre un
   motor no deterministico. La regla presupone el arreglo del punto 1.

3. Runtime 7/10 es generoso. Lo que bajo es el TECHO, no el piso. En los casos F1 pesados
   certificados del V15, el pedido sigue tardando ~23 s, de los cuales ~22.96 s son
   pre-master. El propio V17 lo confirma al ubicar el costo restante en baseline,
   compactacion y generacion en frio.

4. La linea de pools persistidos es la de mayor retorno que queda y NO es cambio de
   heuristica: es memoizacion. Coincide con lo propuesto en la primera revision (compartir
   el cache entre las 40 rondas de generarPatrones en vez de crear un Map por llamada, y
   derivar _sig de un hash estable en vez del orden de aparicion en el subconjunto). Que
   "Compactacion esencialmente rehace trabajo que baseline ya hizo" es la misma observacion
   aplicada a otra etapa. Reutilizar calculo entre las dos es cero regresiones por
   construccion, a diferencia de saltear cualquiera de ellas.

### V18: separacion de fase primaria y refinamiento de remanente

La regla de seguridad es CORRECTA y conviene dejar escrita la demostracion, porque es corta:

    certified = lowerBound >= baselineBoards

Como lowerBound <= optimo <= baselineBoards para toda cota valida, la condicion solo puede
cumplirse con igualdad en los tres. O sea que certificar equivale a probar que el baseline ya
es optimo en placas, y entonces Compactacion no puede bajar de ahi. Diferirla no puede costar
una placa.

Y el caso que podria haber roto la regla no la rompe: si en legacy el baseline daba 8 y
Compactacion lo llevaba a 7 con cota 7, en V18 tenemos LB=7 < baseline=8, no certifica, y cae
al staged completo. La asimetria esta del lado seguro.

### Tres reservas sobre V18

1. NO ES UNA MEJORA DE RUNTIME, ES DE LATENCIA PERCIBIDA. El computo total no baja:
   Compactacion sigue ejecutandose, solo que despues de responder. El informe lo dice
   ("UX/arquitectura"), pero el 8.83% esta presentado de una forma que invita a leerlo como
   speedup. En un servidor el throughput queda igual o peor, porque es el mismo trabajo mas
   coordinacion entre fases.

   Los dos tests directos lo confirman:

       4051587: 3.2 + 6.5 = 9.7 s  contra legacy 10.4 s   -6.7%
       4051461: 2.8 + 6.6 = 9.4 s  contra legacy  8.0 s  +17.5%

   En el segundo el total EMPEORA 17.5%, y ademas el refinamiento fue rechazado, o sea que
   esos 6.6 s no produjeron nada. Con dos casos no se concluye, pero conviene revisar si
   runRemnantRefinement rehace trabajo del baseline al reconstruir estado a partir de
   (lineas, config, primaryPlan). Si lo rehace, ahi esta el 17.5%.

2. LA GANANCIA VUELVE A CAER EN LA COLA BARATA. p50 713 -> 332 ms, p95 y p99 identicos. Es la
   cuarta medicion seguida cuyo beneficio se concentra donde no esta el costo. El diagnostico
   dice que el 87.6% del tiempo vive en 320 casos caros, y esos son exactamente los que no
   reciben nada aca.

   No esta mal hacerlo, pero compite en prioridad con los pools persistidos, que si bajan
   computo real y que el propio V17 identifico como la linea de mayor retorno.

3. RIESGO DE PRODUCTO, EL MAS IMPORTANTE. La seccion 9 dice que exportar/confirmar "puede
   esperar el refinamiento si se desea calidad final garantizada". Ese "si se desea" invierte
   el orden lexicografico.

   Por los datos de la seccion 1, el refinamiento mejora el remanente en 208 de 1182 casos,
   o sea el 17.6%. Si la interfaz permite exportar antes de que termine, en ~1 de cada 6
   pedidos el usuario se lleva un plan con PEOR remanente del que el motor sabe producir. El
   objetivo #2 dice que el remanente le gana al runtime, asi que:

       mostrar temprano: si
       exportar temprano: NO, salvo que el usuario lo pida explicitamente

   El default tiene que ser bloquear la exportacion hasta que el refinamiento termine. De lo
   contrario V18 no es una mejora de UX, es una degradacion silenciosa del objetivo #2
   disfrazada de mejora de UX.

### Sobre el experimento rechazado de la seccion 5

Instrumentar ~42804 empaques para detectar cuales eran reutilizables y terminar 2x mas lento
es motivo suficiente para rechazar. Correcto.

Pero conviene no cerrar por asociacion la idea gruesa, que es distinta y no fue probada:
compartir el cache de patrones entre las 40 rondas de generarPatrones, derivando _sig de un
hash estable de "base|altura|veta" en vez del orden de aparicion en el subconjunto sorteado.
Hoy `opts._cache` se crea DENTRO de cada llamada a optimizar y se tira al terminar la ronda,
y las claves no son comparables entre rondas. Eso es memoizacion pura, sin instrumentacion
por empaque y sin prueba de reutilizabilidad en runtime. Es la linea de pools persistidos.

### Como medir sobre el corpus real

    node scripts/lower-bound-ablation.mjs bounds --train 5000 --holdout 2000 --rebuild
    node scripts/lower-bound-ablation.mjs verify --train 5000 --holdout 2000 --maxNew 200
    node scripts/lower-bound-ablation.mjs report --train 5000 --holdout 2000

El split es el mismo de scripts/experience-benchmark.mjs, asi que los resultados son
comparables con los benchmarks ya registrados. `bounds` solo calcula cotas y es barato.
`verify` corre el motor y es el gate de validez: una sola violacion invalida el modulo.

### Resultado

Modulo disponible y probado. No conectado. El comportamiento productivo del optimizador no
cambio.

### Decision

No conectar la cota al flujo hasta que `verify` cubra el holdout completo con 0 violaciones.
La conexion es un cambio de motor y requiere autorizacion explicita.

### Pendientes

- Correr `verify` sobre el holdout de 2000 y, si da 0 violaciones, sobre el corpus completo
  de 8669. Una sola violacion invalida el modulo entero.
- Medir con `bounds` cuantos de los 320 casos fuera de cota del holdout quedan en cota. Ese
  numero es el valor real del modulo y hoy solo esta estimado en 31.6% sobre datos sinteticos.
- Decidir donde se conecta. `cota` se calcula dentro de optimizarV10 en v10.cjs, que es
  archivo generado y no se edita a mano. Las opciones son pasar la cota por config desde
  legacy-engine.ts y hacer que v10.cjs la use si viene, o mover la compuerta al adaptador TS.
  Cualquiera de las dos toca el motor y necesita autorizacion.
- Toda conexion tiene que quedar detras de un flag `usarCotaFuerte`, apagado por defecto,
  segun la regla de que toda funcionalidad nueva debe poder desactivarse.
- La cota fuerte tambien mejora la poda de armarPlacas (`if(greedy.length<=cota) return greedy`)
  y la compuerta de OneBoard (`cota !== 1`). No medido.
- El costo de la criba crece con el ancho de placa en decimas de mm. Medir el costo del modulo
  por caso antes de conectarlo: si supera unos pocos milisegundos, el perfil fast no lo quiere.
